...
import { Trash2, Plus, Minus, ArrowRight, ShoppingBag, Truck, Store, Calendar, Clock, CheckCircle2, AlertCircle, ChevronLeft, MapPin, MessageSquare } from 'lucide-react';
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          <h2 className="text-3xl font-display font-black text-slate-900">Order Placed!</h2>
          <p className="text-slate-500 font-medium max-w-xs mx-auto">Your order has been sent to the store. We'll notify you once it's confirmed.</p>
        </div>
        ...
        {step === 'cart' ? (
              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    <p className="text-primary font-black text-sm">{formatCurrency(item.price)} <span className="text-[10px] opacity-40 font-bold">/ {item.unit}</span></p>
                    
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center bg-slate-50 rounded-xl p-1 border border-slate-100">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1.5 text-slate-400 hover:text-primary transition-colors"><Minus size={14} /></button>
                        <span className="text-xs font-black w-8 text-center text-slate-900">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1.5 text-slate-400 hover:text-primary transition-colors"><Plus size={14} /></button>
                      </div>
            ...
            <div className="card-premium p-6 flex flex-col gap-4 mb-4">
              ...
              <div className="flex justify-between items-center">
                <div className="flex flex-col">
                  ...
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Excl. delivery fee</span>
                </div>
                <span className="text-2xl font-display font-black text-primary">{formatCurrency(totalPrice)}</span>
              </div>
          ...
          >
            <div className="card-premium p-6 flex flex-col gap-8">
              <div className="flex flex-col gap-4">
                <label className="text-xs font-black uppercase tracking-widest text-slate-400">Delivery Method</label>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    <Truck size={28} strokeWidth={deliveryMethod === 'delivery' ? 2.5 : 2} />
                  ...
                  >
                    <Store size={28} strokeWidth={deliveryMethod === 'pickup' ? 2.5 : 2} />
              ...
              <div className="flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[9px] font-black text-amber-500 uppercase tracking-widest">Delivery fee to be added</span>
                  </div>
                  <span className="text-2xl font-display font-black text-primary">{formatCurrency(totalPrice)}</span>
                </div>
                <p className="text-[10px] text-slate-400 leading-relaxed font-medium">
                  By confirming, you agree to our terms. The store will review your order, set the delivery fee, and confirm your requested schedule.
                </p>
              </div>
