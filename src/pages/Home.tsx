...
import { ShoppingBag, Search, Star, Clock, MapPin, ChevronRight, Flame, Sparkles, Zap, AlertCircle, Heart } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setCategories(cats);
        setPopularProducts(productsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Product)));
      } catch (error) {
  ...
  useEffect(() => {
    if (user) {
      const unsubscribe = favoriteService.subscribeToFavorites(user.uid, setFavoriteIds);
      return unsubscribe;
    } else {
      setFavoriteIds([]);
    }
    ...
    <div className="flex flex-col gap-8 pt-4">
        ...
        <section className="px-5">
          <div className="bg-red-50 border border-red-100 p-4 rounded-3xl flex items-start gap-3 text-red-600">
            ...
            <div className="flex flex-col gap-1">
              <span className="text-xs font-black uppercase tracking-widest">Store on Holiday</span>
              <p className="text-sm font-medium leading-relaxed">
                {storeSettings.holidayMessage || "We are currently closed for the holidays. You can still browse our items!"}
              </p>
            </div>
          </div>
        </section>
      ...
      <section className="px-5 flex flex-col gap-6">
        <div className="flex flex-col">
          ...
          <p className="text-slate-500 font-medium">What are you looking for today?</p>
        </div>
      ...
      <section className="flex flex-col gap-4">
        <div className="px-5 flex justify-between items-center">
          <h2 className="text-lg font-display font-bold text-slate-900">Categories</h2>
          <Link to="/shop" className="text-primary font-bold text-xs flex items-center gap-1">
        ...
        <div className="flex gap-3 overflow-x-auto px-5 pb-2 no-scrollbar">
          ...
          {categories.length === 0 && !loading && (
            <p className="text-slate-400 text-xs font-medium italic px-2">No categories found.</p>
          )}
        </div>
      </section>
        ...
        <div className="relative h-44 bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-premium group">
          ...
          <div className="relative h-full flex flex-col justify-center px-8 gap-2">
            <div className="flex items-center gap-2 bg-primary/20 backdrop-blur-md border border-primary/30 px-3 py-1 rounded-full w-fit">
              ...
              <span className="text-primary text-[10px] font-bold uppercase tracking-widest">Hot Deal</span>
            </div>
            <h3 className="text-2xl font-display font-black text-white leading-tight">
              Free Delivery <br/> on your first order!
            </h3>
            <Link to="/shop" className="text-white font-bold text-xs underline underline-offset-4 mt-1">Shop Now</Link>
          </div>
        </div>
      </section>
      ...
      <section className="flex flex-col gap-6">
        <div className="px-5 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-display font-bold text-slate-900">Popular Now</h2>
          </div>
          <Link to="/shop" className="text-primary font-bold text-xs">View All</Link>
        </div>
      ...
      <section className="px-5 mb-4">
        <div className="bg-white rounded-[2.5rem] p-8 shadow-soft border border-slate-50 flex flex-col gap-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Our Location</span>
              <span className="font-display font-bold text-slate-900">
                {storeSettings?.address || 'Corner of 5th Ave.'}
              </span>
            </div>
          </div>
          <div className="flex flex-col gap-2">
            <h3 className="text-xl font-display font-black text-slate-900 leading-tight">Always here for you.</h3>
            <div className="flex flex-col gap-1">
              {todayHours ? (
                <div className="flex items-center gap-2">
                ...
                <p className="text-slate-500 text-sm leading-relaxed">Open daily from 6:00 AM to 10:00 PM. We're your neighborhood digital sari-sari store.</p>
              )}
              <p className="text-slate-500 text-[11px] font-medium italic">We're your neighborhood digital sari-sari store.</p>
            </div>
          </div>
          <div className="flex items-center gap-4 pt-2">
            ...
            <span className="text-[11px] font-bold text-slate-400 underline underline-offset-2">500+ happy neighbors</span>
          </div>
        </div>
      </section>
    </div>
