      ...
      async (snapshot) => {
        ...
        const ordersWithNames = await Promise.all(
          rawOrders.map(async (order) => {
            // 1. If customerName exists and isn't "Online Customer", keep it
            if (order.customerName && order.customerName !== "Online Customer")
              return order;
            ...
            if (order.userId) {
              try {
                ...
                if (userSnap.exists()) {
                  ...
                  // Check all common name fields in Firebase
                  const fetchedName =
                    u.displayName || u.name || u.fullName || u.email;

                  if (fetchedName) {
                    return { ...order, customerName: fetchedName };
                  }
                }
              } catch (err) {
          ...
          {showNotifications && (
            ...
            >
              ...
              <div className="flex flex-col gap-3 max-h-[300px] overflow-y-auto pr-1">
                ...
                {orders.length === 0 && (
                  <p className="text-center py-4 text-slate-400 text-xs font-medium">
                    No recent activity
                  </p>
                )}
              </div>
          ...
          <div className="flex flex-col gap-4">
            {recentOrders.length > 0 ? (
              recentOrders.map((order) => {
                // FALLBACK LOGIC:
                // 1. Check for customerName (now fetched from users collection)
                // 2. Check for buyerNotes split
                // 3. Fallback to Guest
                const buyerName = order.customerName
                  ? order.customerName
                  : order.buyerNotes
                    ? order.buyerNotes.split(" - ")[0]
                    : `Guest #${order.id.slice(-4).toUpperCase()}`;
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs font-medium text-slate-700 mt-1 bg-amber-50 px-2 py-0.5 rounded-full max-w-[160px] truncate border border-amber-100/50">
                          {buyerName}
                        </span>
              ...
              <div className="p-10 text-center card-premium text-slate-400 font-medium">
                No orders yet
              </div>
            )}
          </div>
