...
import { Clock, CheckCircle2, XCircle, ShoppingBag, User, Trash2, AlertCircle, Truck, Store, Calendar, MapPin, MessageSquare, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function AdminOrders() {
  ...
  const updateStatus = async (orderId: string, status: Order['status'], extraData: any = {}) => {
    if (!db) return;
    try {
  ...
  const submitConfirmation = async () => {
    ...
    await updateStatus(confirmingOrder.id, 'confirmed', {
      ...
      confirmedTime,
      sellerNotes,
      deliveryFee: confirmingOrder.deliveryMethod === 'delivery' ? (parseFloat(deliveryFee) || 0) : 0
    });
  };
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'out_for_delivery': return 'bg-emerald-50 text-emerald-600 border-emerald-100';
  ...
  return (
    <div className="pb-24 pt-6 px-5 flex flex-col gap-8">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-slate-500 text-sm font-medium">Review and process customer orders.</p>
      </header>
              ...
              >
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-2">
                      <span className={cn(
                        ...
                        order.type === 'pos' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-blue-50 text-blue-600 border-blue-100'
                      )}>
                        {order.type}
                      </span>
                      <span className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">
                        {order.timestamp?.toDate ? order.timestamp.toDate().toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' }) : 'Just now'}
                      </span>
                    </div>
                    <h3 className="font-black text-slate-900 text-lg">Order #{order.id.slice(-6).toUpperCase()}</h3>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className={cn("badge-status border", getStatusColor(order.status))}>
                ...
                <div className="grid grid-cols-2 gap-4 items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-sm font-black text-slate-800 truncate max-w-[120px]">{order.customerName || 'Online Buyer'}</span>
                    </div>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                      {order.deliveryMethod === 'delivery' && order.deliveryFee ? 'Subtotal' : 'Total Amount'}
                    </span>
                    <span className={cn(
                      "font-black tracking-tight",
                      (order.deliveryMethod === 'delivery' && order.deliveryFee) ? "text-lg text-slate-600" : "text-2xl text-primary"
                    )}>
                      {formatCurrency(order.total)}
                    </span>
                    {order.deliveryMethod === 'delivery' && order.deliveryFee !== undefined && order.deliveryFee > 0 && (
                      <div className="flex flex-col items-end mt-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Delivery Fee</span>
                          <span className="text-sm font-black text-slate-600">{formatCurrency(order.deliveryFee)}</span>
                        </div>
                        <div className="flex items-center gap-2 mt-1 pt-1 border-t border-slate-100">
                          ...
                          <span className="text-2xl font-black text-primary">{formatCurrency(order.total + order.deliveryFee)}</span>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-slate-50/80 p-4 rounded-3xl flex flex-col gap-3 border border-slate-100">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-[11px] font-black text-slate-600 uppercase tracking-wider">
                        {order.deliveryMethod === 'delivery' ? <Truck size={14} className="text-primary" /> : <Store size={14} className="text-primary" />}
                    ...
                    {order.deliveryMethod === 'delivery' && order.deliveryAddress && (
                      <div className="flex items-start gap-2 p-3 bg-white rounded-2xl border border-slate-100">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs font-bold text-slate-700 leading-relaxed">{order.deliveryAddress}</span>
                        </div>
                      </div>
                    )}
                    {order.buyerNotes && (
                      <div className="flex items-start gap-2 p-3 bg-white rounded-2xl border border-slate-100">
                        ...
                        <div className="flex flex-col">
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Buyer Instructions</span>
                          <span className="text-xs font-bold text-slate-700 leading-relaxed">{order.buyerNotes}</span>
                        </div>
                      </div>
                    )}
                    {order.confirmedDate && (
                      <div className="text-[11px] text-emerald-600 font-black flex items-center gap-1.5 bg-emerald-50/50 p-2 rounded-xl border border-emerald-100/50">
                        ...
                        Confirmed for: {order.confirmedDate} at {order.confirmedTime}
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  ...
                  {order.status === 'preparing' && (
                    <motion.button 
                      whileTap={{ scale: 0.98 }}
                      onClick={() => updateStatus(order.id, order.deliveryMethod === 'delivery' ? 'out_for_delivery' : 'ready_for_pickup')}
          ...
          {orders.length === 0 && (
            <div className="py-24 text-center flex flex-col items-center gap-6">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-slate-400 font-medium text-sm">When customers buy, they'll appear here.</p>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
            ...
            >
              <div className="flex flex-col gap-2">
                <h3 className="text-2xl font-black text-slate-900 tracking-tight">Confirm Order</h3>
                <p className="text-slate-500 text-sm font-medium">Set the final schedule and add notes for the buyer.</p>
              </div>

              <div className="flex flex-col gap-5">
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Confirmed Date</label>
                    <input 
                      type="date" 
                      value={confirmedDate}
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Confirmed Time</label>
                    <input 
                      type="time" 
                      value={confirmedTime}
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Seller Notes (Optional)</label>
                  <textarea 
                    value={sellerNotes}
          ...
          <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6">
            ...
            >
              <div className="flex flex-col gap-2">
                ...
                <h3 className="text-2xl font-black text-slate-900 tracking-tight">Delete Order?</h3>
                <p className="text-slate-500 text-sm font-medium">This will permanently remove this order record.</p>
              </div>
              <div className="flex flex-col gap-3">
                <button 
